...
int main(){
    ...
    for(int i=0;i<n;i++){
        printf("P%d\t\t%d %d %d\t\t\t%d %d %d\t\t%d %d %d\n",(i+1), (processes+i)->allocation[0],(processes+i)->allocation[1],(processes+i)->allocation[2],(processes+i)->max[0],(processes+i)->max[1],(processes+i)->max[2],cpu_work_units[0], cpu_work_units[1], cpu_work_units[2]);
    }
